...
BYMCode <- nimbleCode({
  ...
  for (i in 1:N.non_islands){
    ...
    y[i] ~ dpois(mu[i])
    log(mu[i]) <- log(e[i]) + b0 + b1*aff[i] + s[i] + u[i]
    u[i] ~ dnorm(0,tau.u)
    
    #area-specific SIR
    SIR[i] <- exp(b0 + b1*aff[i] + s[i] + u[i])
...
monitors <- c("b0", "b1", "allSIR", "resSIR", "SIR", "SIR.i",
              "res", "s", "var.s", "mu", "var.total")

##Setup for NIMBLE model

BYMConsts <- list(N.islands = N.islands,
                     N.non_islands = N.no_islands,
                     L = length(LipAdj$adj),
                     adj = LipAdj$adj, w = LipAdj$weights, num = LipAdj$num)

BYMData <- list(y.i = cases.islands, aff.i = aff.islands,
                   e.i = E.islands,
...
BYMInits <- list(
  list(b0 =  0, b1 = 0, u.i = rnorm(N.islands,0,1),
       s = rnorm(N.no_islands,0,1), tau.s = 1,
       u = rnorm(N.no_islands,0,1), tau.u = 1),   #chain 1
  
  list(b0 =  1, b1 = 1, u.i = rnorm(N.islands,0,1),
       s = rnorm(N.no_islands,0,1), tau.s = 1,
       u = rnorm(N.no_islands,0,1), tau.u = 1),   #chain 2
  
  list(b0 =  1, b1 = 5, u.i = rnorm(N.islands,0,1),
       s = rnorm(N.no_islands,0,1), tau.s = 1,
       u = rnorm(N.no_islands,0,1), tau.u = 1),   #chain 3
  
  list(b0 = -1, b1 = 5, u.i = rnorm(N.islands,0,1),
       s = rnorm(N.no_islands,0,1), tau.s = 1,
       u = rnorm(N.no_islands,0,1), tau.u = 1)   #chain 4
...
BYMCodesamples$summary$all.chains[c('b0', 'b1', 'var.total'),]
...
d$SIRposterior <- BYMCodesamples$summary$all.chains[paste0("SIR[", 1:N.no_islands, "]"), "Median"]
d.islands$SIRposterior <- BYMCodesamples$summary$all.chains[paste0("SIR.i[", 1:N.islands, "]"), "Median"]
